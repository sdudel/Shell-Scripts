...
function yum_repos {
	...
	yum install wget -y >> /var/log/ss.log 2>&1
}
...
function config_vim {
	...
	echo 'Setting dark vim color scheme...'
	echo 'colorscheme dark' >> /etc/vimrc
...
function remove_crapware {
	...
	yum -y remove squid exim sendmail krb5-workstation cups sendmail irda-utils mt-st samba-common sendmail-cf talk up2date ypbind yp-tools wvdial lockdev procmail xorg-x11-font-utils pam_ccreds gdm bluez-utils yum-updatesd libX11  >> /var/log/initial_setup.log 2>&1
...
function setup_server {
 ...
 echo ‘All Done!  A log file has been generated at /var/log/ss.log'
 echo 'Please reboot...'
...
echo '==========================='
echo 'Setup for only fresh and clean CentOS/RHEL box'
